...
class JooCaptcha {
 ...
 function __construct(){
   ...
   function CaptchaSecurityImages($width='100',$height='30',$characters='6',$backgroundColor=array( 'red' =>255, 'green' => 255, 'blue' => 255),$foregroundColor=array( 'red' =>255, 'green' => 40, 'blue' => 100)) {
   	  $this->destroy();
     ...
     // $text_color = imagecolorallocate($image, 255, 40, 100);
      ...
      $background_color = imagecolorallocate($image, $backgroundColor['red'], $backgroundColor['green'], $backgroundColor['blue']);
      $text_color = imagecolorallocate($image, $foregroundColor['red'], $foregroundColor['green'], $foregroundColor['blue']);
		$app= JFactory::getApplication();
		$params= $app->getParams();
		$noiseType=$params->get( 'captcha_noise_type', '0' );
		if($noiseType!=0){
			$noise_color = $text_color;
			if($noiseType==1){//dot types
				for( $i=0; $i<($width*$height)/3; $i++ ) {
					imagefilledellipse($image, mt_rand(0,$width), mt_rand(0,$height), 1, 2, $noise_color);
				}
			}else if($noiseType==2){ //line types
