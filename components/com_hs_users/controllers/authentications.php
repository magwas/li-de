...
class Hs_usersControllerAuthentications extends JControllerLegacy {
	...
	public function authenticatewith() {
		//check token	
		//FT 2014.12.01 valami gond van ezzel. :(
    // JSession::checkToken('get') or jexit(JText::_('JINVALID_TOKEN'));
		...
		if(mb_strlen($d->email)>0&&mb_strlen($d->display_name)>0){
			...
			if($isNewUser){
				...
				$ck = $jmodelRegistration->register(array('password1'=>$password, 'password2'=>$password));
				
				if($ck==false){
					$errorMsg=$jmodelRegistration->getError();
				}
			}else{
				/* TODO: There is no need to edit profile for registered user?
				//starts com_users profile edit stream
				require_once JPATH_SITE.'/components/com_users/models/profile.php';	
				$jmodelProfile = $this->getModel('Profile', 'UsersModel');			
				$profileData = $modelUser->getJProfileData();
					
				//add password to array
				$profileData['passwaord1'] = 	$password;
				$profileData['passwaord2'] = 	$password;		
				$ck = $jmodelProfile->save($profileData);	
				if($ck==false){
					$errorMsg=$jmodelProfile->getError();
				}				
				*/		
