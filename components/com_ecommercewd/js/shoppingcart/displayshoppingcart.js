...
function wdShop_updateOrderProduct(jq_productContainer) {
    ...
    jq_productContainer.find(".wd_shop_order_product_parameter").each(function () {
        ...
        switch (parameterTypeId) {
            ...
            case '3':
				...
				select_parameter_price = select_parameter_price.replace(/,/g, "");
            ...
            case '4':
				...
				radio_parameter_price = radio_parameter_price.replace(/,/g, "");
            ...
            case '5':
                ...
                jQuery('input[name=' + parameter_checkbox_name + ']:checked').each(function () {
                    ...
                    if (price) {
                        if (sign == '+') {
                            parameter_checkbox_price = (parseFloat(parameter_checkbox_price) + parseFloat(price)).toFixed(decimals);
                        } else {
                            parameter_checkbox_price = (parseFloat(parameter_checkbox_price) - parseFloat(price)).toFixed(decimals);
                        }
                    }
        ...
        if (parameter_price_text.length != 0 && parameter_price_text != 0) {
            ...
            if (price) {
                if (sign == '+') {
                    parameters_price = (parseFloat(parameters_price) + parseFloat(price)).toFixed(decimals);
                } else {
                    parameters_price = (parseFloat(parameters_price) - parseFloat(price)).toFixed(decimals);
                }
            }
    ...
    wdShop_orderProductRequestsMap[orderProductId] = jQuery.ajax({
        type: "POST",
        url: wdShop_urlUpdateOrderProduct + "&order_product_id=" + orderProductId,
        data: {
        ...
        success: function (result) {
            ...
            jq_productContainer.find(".wd_shop_order_product_quantity").val(data.product_count);
			jq_productContainer.find(".wd_shop_order_product_final_price").html(data.product_final_price_text);
            jq_productContainer.find(".wd_shop_order_product_final_price_info").attr('data-original-title', '<span>Price: ' + data.product_final_price_text + '</span> <br> ' + products_data[data.order_product_id]['final_price_info'] );
       
            jq_productContainer.find(".wd_shop_order_product_subtotal").html(data.product_subtotal_text);
...
function wdShop_removeAllOrderProducts(jq_btnRemoveAll) {
    ...
    if (!jQuery.support.leadingWhitespace) {
        ishover = jq_btnRemoveAll.hasClass("hover");
    }
    else {
        ishover = jq_btnRemoveAll.is(":hover");
    }
    if (ishover) {
...
function addParametersPrices() {
    for (var i in  product_parameters_price) {
        ...
        for (var j in parameters_price) {
            ...
            if (param_price) {
                if (sign == '+') {
                    product_parameters_price_sum = (parseFloat(product_parameters_price_sum) + parseFloat(param_price)).toFixed(2);
                } else {
                    product_parameters_price_sum = (parseFloat(product_parameters_price_sum) - parseFloat(param_price)).toFixed(2);
                }
            }
