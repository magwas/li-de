...
jQuery(document).ready(function () {
    ...
    // email
    var jq_filedEmail = jq_mainForm.find("input[name=billing_data_email]");
...
function wdShop_checkout_getInvalidFields() {
    ...
    // email
    var jq_fieldEmail = jq_mainForm.find("input[name=billing_data_email]");
 ...
 function wd_ShopCopyBillingInformation(event,obj){
	...
	if(jQuery(obj).prop('checked') == true){
		for(i=0; i<wdShop_billing_fields.length; i++){			
			jQuery('#shipping_data_' + wdShop_billing_fields[i]).val(jQuery('#billing_data_' + wdShop_billing_fields[i]).val());
		}
	}
