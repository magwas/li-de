...
<form name="adminForm" id="adminForm" action="" method="POST">
    ...
    <table class="adminlist table table-striped">
        ...
        for ($i = 0; $i < count($rows); $i++) {
            $row = $rows[$i];
            ?>
            <tr class="row<?php echo $i % 2; ?>" data-type="<?php echo $row->short_name;?>">
				...
				<td class="col_image">
					<?php if($row->short_name!='without_online_payment'):?>
						<a href="<?php echo $row->edit_url; ?>" class="img_url">
							<img src="<?php echo JURI::base().'components/com_'.WDFHelper::get_com_name().'/images/payments/'.$row->base_name.'.png' ;?>" width="75"/>
						</a>
					<?php endif;?>	
                </td>
