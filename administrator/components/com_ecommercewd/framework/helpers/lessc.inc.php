...
class lessc {
    ...
    static public function compressList($items, $delim) {
        if (!isset($items[1]) && isset($items[0])) return $items[0]; else return array('list', $delim, $items);
    }
    ...
    protected function compileProp($prop, $block, $out) {
        ...
        switch ($prop[0]) {
                ...
                foreach ($mixins as $mixin) {
                    ...
                    foreach ($this->sortProps($mixin->props) as $subProp) {
                        if ($suffix !== null && $subProp[0] == "assign" && is_string($subProp[1]) && $subProp[1]{0} != $this->vPrefix
                            ...
                            $subProp[2] = array('list', ' ', array($subProp[2], array('keyword', $suffix)));
                        }
            ...
            case "import";
                ...
                $this->env->imports[$importId] = $result === false ? array(false, "@import " . $this->compileValue($importPath) . ";") : $result;
    ...
    protected function lib_mix($args) {
        if ($args[0] != "list" || count($args[2]) < 2) $this->throwError("mix expects (color1, color2, weight)");
        ...
        switch ($value[0]) {
            ...
            case "function":
                ...
                if (is_callable($f)) {
                    ...
                    if (is_numeric($ret)) $ret = array('number', $ret, ""); elseif (!is_array($ret)) $ret = array('keyword', $ret);
        ...
        switch ($value[0]) {
            ...
            case 'keyword':
                ...
                if (isset(self::$cssColors[$name])) {
                    ...
                    if (isset($rgba[3])) return array('color', $rgba[0], $rgba[1], $rgba[2], $rgba[3]);
