...
class UploadHandler {
    ...
    function __construct($options = null, $uploaded_type = null, $initialize = true, $error_messages = null) {
	
        $this->options = array('script_url' => $this->get_full_url() . '/', 'upload_dir' => dirname($_SERVER['SCRIPT_FILENAME']) . '/files/', 'upload_url' => $this->get_full_url() . '/files/', 'user_dirs' => false, 'mkdir_mode' => 0755, 'param_name' => 'files', // Set the following option to 'POST', if your server does not support
            ...
            'download_via_php' => false, // Defines which files can be displayed inline when downloaded:
            'inline_file_types' => '/\.(gif|jpe?g|JPG|png)$/i', // Defines which files (based on their names) are accepted for upload:
            ...
            'max_number_of_files' => null, // Image resolution restrictions:
            'max_width' => null, 'max_height' => null, 'min_width' => 1, 'min_height' => 1, // Set the following option to false to enable resumable uploads:
            'discard_aborted_uploads' => true, // Set to true to rotate images based on EXIF meta data, if available:
            'orient_image' => false, 'image_versions' => array( // Uncomment the following version to restrict the size of
                // uploaded images:
                /*
                '' => array(
                    'max_width' => 1920,
                ...
                */
                // Uncomment the following to create medium and original sized images:
                /*
                'medium' => array(
                    'max_width' => 800,
                    'max_height' => 600,
		...
		if ($uploaded_type) {
            $this->uploaded_type = $uploaded_type;			
        }
    ...
    protected function get_upload_path($file_name = null, $version = null) {		
		...
		if($this->uploaded_type == "downloadable_files"){
			 return $this->options['upload_dir'] . $this->get_user_path() . md5($this->options['upload_dir'] . $this->get_user_path() . $file_name);
		}
...
//        switch (strtolower(substr(strrchr($file_name, '.'), 1))) {
    ...
    protected function trim_file_name($name, $type, $index, $content_range) {
        ...
        if (strpos($name, '.') === false && preg_match('/^image\/(gif|jpe?g|JPG|png)/', $type, $matches)
        ) {
            $name .= '.' . $matches[1];
        }
        return $name;
    ...
    public function get($print_response = true) {
        ...
        $file_name = $this->get_file_name_param();
        if ($file_name) {
            $response = array(substr($this->options['param_name'], 0, -1) => $this->get_file_object($file_name));
        } else {
