...
// fix for json functions for older versions of php
...
class WDFJson {
    ...
    public static function decode($json_str, $options = 0, $assoc = false, $depth = 512) {
		$json_str = get_magic_quotes_gpc() ? stripslashes($json_str) : $json_str;
        try {
            $obj = json_decode($json_str, $assoc, $depth, $options);
        } catch (Exception $exception) {
