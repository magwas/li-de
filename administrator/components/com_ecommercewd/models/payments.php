...
class EcommercewdModelPayments extends EcommercewdModel {
    ...
    public function get_rows() {
		...
		// additional data
		foreach($rows as $row){
		    $row->edit_url = 'index.php?option=com_' . WDFHelper::get_com_name() . '&controller=' . WDFInput::get_controller() . '&task=edit' . '&cid[]=' . $row->id . '&type='. $row->short_name;
		}
			...
			foreach($fields as $k => $field){				
				if($field['type'] == 'radio'){
					...
					foreach( $field['options'] as $key => $value )
						$radio_list[$k][] = (object)array('value' => $key, 'text' => WDFText::get($value));
				}		
			}				
