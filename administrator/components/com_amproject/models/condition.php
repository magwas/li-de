...
*/
 defined('_JEXEC') or die('Restricted access');
/**
 * AmprojectModelCondition 
 * @author Fogler Tibor
 */
 
 
class AmprojectModelCondition  extends AmprojectModel { 

	
	
	protected $_default_filter = 'a.table_type';   
 ...
 */
	...
	*/
	...
	protected function _buildContentWhere() 
	{
		...
		if ($search) {
			$this->_query->where('LOWER(a.table_type) LIKE ' . $this->_db->Quote('%' . $search . '%'));			
		}
    
    $table = JRequest::getVar('table','');
    if ($table != '') {
			$this->_query->where('a.table_type="#__ampm_'.JRequest::getVar('table').'" and a.table_id = "'.JRequest::getVar('tableid').'"');			
    }
	}
