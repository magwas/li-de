...
if ($wfshema != '') {
  ...
  if (($wfshema != '') & (count($this->items)>0)) {
    $owners = $model->getOwners('#__ampm_taskshema',$this->items[0]->id);
  } else {
    $owners = array();
  }    
...
<form action="index.php?option=com_amproject&amp;view=taskshema" method="post" name="adminForm" id="adminForm">
	<table>
		<tr>
...
<div id="editcell">
	<table class="adminlist table table-striped">
        ...
        <?php 
          if ($wfshema != '') 
               $this->pagination->setAdditionalUrlParam('wfshema',$wfshema);
...
<?php
  ...
  for ($i=0, $n=count( $this->items ); $i < $n; $i++):
    ...
    if (JRequest::getVar('function', null)) {
    	$onclick= "onclick=\"window.parent.jSelectTaskshema_id('".$row->id."', '".$this->escape($row->title)."', '','id')\" ";
    }  	
    
 	$link = JRoute::_( 'index.php?option=com_amproject&view=taskshema&task=edit&cid[]='. $row->id.'&wfshema='.$wfshema );
 	$linkPublish = JRoute::_( 'index.php?option=com_amproject&view=taskshema&cid[]='. $row->id.'&wfshema='.$wfshema );
 	$row->id = $row->id; 	
...
<input type="hidden" name="task" value="taskshema" />
<input type="hidden" name="view" value="taskshema" />
<input type="hidden" name="boxchecked" value="0" />
<input type="hidden" name="filter_order" value="<?php echo $this->lists['order']; ?>" />
<input type="hidden" name="filter_order_Dir" value="" />
<input type="hidden" name="wfshema" value="<?php echo JRequest::getVar('wfshema',''); ?>" />
