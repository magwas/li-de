...
defined('_JEXEC') or die('Restricted access');
  ...
  JToolBarHelper::custom('wfshemas','wfshemas','','wfshemas'); 
  JToolBarHelper::custom('events','events','','events'); 
  JToolBarHelper::preferences('com_amproject', '550');  
?>
<form action="index.php?option=com_amproject&amp;view=product" method="post" name="adminForm" id="adminForm">
	<table>
		<tr>
...
<?php
  ...
  for ($i=0, $n=count( $this->items ); $i < $n; $i++):
    ...
    if (JRequest::getVar('function', null)) {
    	$onclick= "onclick=\"window.parent.jSelectProduct_id('".$row->id."', '".$this->escape($row->title)."', '','id')\" ";
    }  	
    
 	$link = JRoute::_( 'index.php?option=com_amproject&view=product&task=edit&cid[]='. $row->id );
 	$linkPublish = JRoute::_( 'index.php?option=com_amproject&view=product&cid[]='. $row->id );
 	$row->id = $row->id; 	
