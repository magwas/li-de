...
defined('_JEXEC') or die('Restricted access');
  ...
  JToolBarHelper::title( JText::_( 'Project' ), 'generic.png' );
...
<form action="index.php?option=com_amproject&amp;view=project" method="post" name="adminForm" id="adminForm">
	<table>
		<tr>
...
<?php
  ...
  for ($i=0, $n=count( $this->items ); $i < $n; $i++):
    ...
    if (JRequest::getVar('function', null)) {
    	$onclick= "onclick=\"window.parent.jSelectProject_id('".$row->id."', '".$this->escape($row->title)."', '','id')\" ";
    }  	
    
 	$link = JRoute::_( 'index.php?option=com_amproject&view=project&task=edit&cid[]='. $row->id );
 	$row->id = $row->id; 	
