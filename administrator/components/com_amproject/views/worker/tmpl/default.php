...
defined('_JEXEC') or die('Restricted access');
  ...
  JToolBarHelper::title( JText::_( 'Worker' ), 'generic.png' );
...
<form action="index.php?option=com_amproject&amp;view=worker" method="post" name="adminForm" id="adminForm">
	<table>
		<tr>
		...
		<thead>
			<tr>
				...
				<th class="title">
					...
					<?php echo JHTML::_('grid.sort', 'Nick', 'a.nick', $this->lists['order_Dir'], $this->lists['order'] ); ?>
				</th>								<th class="title">
...
<?php
  ...
  for ($i=0, $n=count( $this->items ); $i < $n; $i++):
    ...
    if (JRequest::getVar('function', null)) {
    	$onclick= "onclick=\"window.parent.jSelectWorker_id('".$row->id."', '".$this->escape($row->name)."', '','id')\" ";
    }  	
    
 	$link = JRoute::_( 'index.php?option=com_amproject&view=worker&task=edit&cid[]='. $row->id );
 	$row->id = $row->id; 	
...
<input type="hidden" name="task" value="worker" />
<input type="hidden" name="view" value="worker" />
