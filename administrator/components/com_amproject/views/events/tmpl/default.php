...
<form action="index.php?option=com_amproject&amp;view=events" method="post" name="adminForm" id="adminForm">
	<table>
		<tr>
	...
	<table class="adminlist table table-striped">
		<thead>
			<tr>
				...
				<th class="title">
					<?php echo JHTML::_('grid.sort', 'Tablename', 'a.tablename', $this->lists['order_Dir'], $this->lists['order'] ); ?>
				</th>
				<th class="title">
					<?php echo JHTML::_('grid.sort', 'Time', 'a.time', $this->lists['order_Dir'], $this->lists['order'] ); ?>
				</th>
        <th class="title">
...
<?php
  ...
  for ($i=0, $n=count( $this->items ); $i < $n; $i++):
    ...
    if (JRequest::getVar('function', null)) {
    	$onclick= "onclick=\"window.parent.jSelectEvents_id('".$row->id."', '".$this->escape($row->tablename)."', '','id')\" ";
    }  	
    
 	$link = JRoute::_( 'index.php?option=com_amproject&view=events&task=edit&cid[]='. $row->id );
 	$row->id = $row->id; 	
  ...
  ?>
    ...
    <td><?php echo $checked  ?></td>	
     <td>
							<a <?php echo $onclick; ?>href="<?php echo $link; ?>"><?php echo $row->tablename; ?></a>
		</td>
