 ...
 * @since		1.6
 */
class JFormFieldjdCustomField12 extends JFormField
{
	/**
	 * The form field type.
	 *
	 ...
	 * @since	1.6
	 */
	protected $type = 'jdCustomField12';
    ...
    public function getLabel() {
        ...
        if ($app->isAdmin()){
            $text = $this->element['label'] ? (string) $this->element['label'] : (string) JDownloadsHelper::getOnlyLanguageSubstring($jlistConfig['custom.field.12.title']);
        } else {
            $text = $this->element['label'] ? (string) $this->element['label'] : (string) JDHelper::getOnlyLanguageSubstring($jlistConfig['custom.field.12.title']);
        }  
