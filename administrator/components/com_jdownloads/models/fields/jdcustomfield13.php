 ...
 * @since		1.6
 */
class JFormFieldjdCustomField13 extends JFormField
{
	/**
	 * The form field type.
	 *
	 ...
	 * @since	1.6
	 */
	protected $type = 'jdCustomField13';
    ...
    public function getLabel() {
        ...
        if ($app->isAdmin()){
            $text = $this->element['label'] ? (string) $this->element['label'] : (string) JDownloadsHelper::getOnlyLanguageSubstring($jlistConfig['custom.field.13.title']);
        } else {
            $text = $this->element['label'] ? (string) $this->element['label'] : (string) JDHelper::getOnlyLanguageSubstring($jlistConfig['custom.field.13.title']);
        }        
