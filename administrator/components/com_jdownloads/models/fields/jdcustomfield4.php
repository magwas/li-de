 ...
 * @since		1.6
 */
class JFormFieldjdCustomField4 extends JFormFieldList
{
	/**
	 * The form field type.
	 *
    ...
    public function getLabel() {
        ...
        $text = $this->element['label'] ? (string) $this->element['label'] : (string) $jlistConfig['custom.field.4.title'];;
	...
	protected function getOptions()
	{
        ...
        if ($jlistConfig['custom.field.4.title'] != ''){
            if ($app->isAdmin()){
                $values = explode(',', JDownloadsHelper::getOnlyLanguageSubstring($jlistConfig['custom.field.4.values']));
            } else {
                $values = explode(',', JDHelper::getOnlyLanguageSubstring($jlistConfig['custom.field.4.values']));
            }    
        }
