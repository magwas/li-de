<?php
/**
 * @version		$Id: //depot/dev/Joomla/Joo_Comments/ver_1_0_0/com_joocomments/admin/models/settings.php#2 $
 ...
 * @subpackage	com_config
 */
class JooCommentsModelSettings extends JModelForm
{
	/**
	...
	protected function populateState()
	{
		...
		$this->setState('component.option','com_joocomments');
	...
	function retriveData($parentId,$recursive=false){
		...
		// Select some fields
		$query->select('con.title,con.attribs');
		// From the hello table
		$query->from('#__content as con');
		$query->where('con.catid='.$parentId.'AND con.published=1');
		$query->order('con.id');
		$db->setQuery($query);
